(function(){
  ...
  function arm(){
    startObserver();
    sweepHostile();
    // Continuous sweep ~45s only — lighter than before (less main-thread lag)
    if(window.__keenHostileTimer) clearInterval(window.__keenHostileTimer);
    var n=0;
    window.__keenHostileTimer=setInterval(function(){
      sweepHostile();
      n++;
      if(n>60){ // 60 * 750ms ≈ 45s
  ...
  window.__keenHostileV4={sweep:sweepHostile,arm:arm};
  window.__keenHostileV3=window.__keenHostileV4;
  window.__keenHostileV2=window.__keenHostileV4;
  arm();
  document.addEventListener('DOMContentLoaded',function(){ arm(); },{once:true});
  window.addEventListener('load',function(){ arm(); },{once:true});
  return window.__keenHostileV4;
})();
